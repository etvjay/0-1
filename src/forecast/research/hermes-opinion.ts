...
const validate = (value: StructuredOpinion, allowedEvidence: Set<string>): StructuredOpinion => {
  if (!Number.isFinite(value.probability) || value.probability < 0 || value.probability > 1) throw new Error("Hermes opinion probability invalid");
  if (!Number.isFinite(value.confidence) || value.confidence < 0 || value.confidence > 1) throw new Error("Hermes opinion confidence invalid");
...
export class HermesOpinionProvider implements OpinionProvider {
  readonly name = "hermes-opinion-v1";
  ...
  async forecast(request: OpinionRequest): Promise<ForecastOpinion> {
    ...
    const system = [
      ...
      roleInstruction,
      "Use only the supplied evidence in this step; do not execute trades or make capital decisions.",
      "Return only JSON with probability, confidence, evidence_ids, assumptions, rationale.",
