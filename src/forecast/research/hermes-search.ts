...
export class HermesSearchProvider implements SearchProvider {
  readonly name = "hermes-web-search-v3";
  ...
  async search({ query }: SearchRequest): Promise<SearchResultRecord[]> {
    const system = [
      ...
      "Use web_search exactly once for this request.",
      "Do NOT call web_extract, browser, terminal, file, code execution, skills, memory, delegation, cron, image, or any other tool.",
      "If web_search fails or returns no usable results, immediately return {\"results\":[]}.",
    ...
    const user = JSON.stringify({
      ...
      maxResults: Math.min(query.maxResults, 4),
    });
    ...
    return parsed.results.slice(0, Math.min(query.maxResults, 4)).flatMap((item) => {
